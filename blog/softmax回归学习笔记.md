...
- $K$：类别总数（本次为 10）
- 分母 $\sum_k \exp(o_k)$：保证所有输出之和为 1，即概率分布
...
其中：
- $\mathbf{X}$ 形状：$(n, 784)$，$n$ 为 batch size
- $\mathbf{W}$ 形状：$(784, 10)$，将 784 维输入映射到 10 个类别
- $\mathbf{b}$ 形状：$(10,)$
- $\mathbf{o}$ 形状：$(n, 10)$，每行是一张图片的 10 个类别评分
...
预测越准确，$\hat{\mathbf{y}}$ 越接近 $\mathbf{y}$，梯度越小。这个非常形象——预测错了梯度大（大幅修正），预测对了梯度小（微调即可）。
